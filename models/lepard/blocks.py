...
from models.lepard.kernel_points import load_kernels
...
class UnaryBlock(nn.Module):
    ...
    def __repr__(self):
        return 'UnaryBlock(in_feat: {:d}, out_feat: {:d}, BN: {:s}, LeakyReLU: {:s})'.format(self.in_dim,
                                                                                        self.out_dim,
                                                                                        str(self.use_bn),
...
class SimpleBlock(nn.Module):
    ...
    def forward(self, x, batch):
        ...
        assert not torch.any(torch.isnan(q_pts))
        assert not torch.any(torch.isnan(s_pts))
        assert not torch.any(torch.isnan(neighb_inds))
        assert not torch.any(torch.isnan(x))
